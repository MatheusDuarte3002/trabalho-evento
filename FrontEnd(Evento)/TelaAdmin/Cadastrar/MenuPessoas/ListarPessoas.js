window.onload = function() {
  fetch('http://localhost:3001/usuarios')
    ...
    .then(data => {
      const table = document.querySelector('.table-listarpessoa');
      data.forEach(user => {
        if (user.tipo === 'pessoa') {
        ...
        deleteButton.addEventListener('click', function() {
          if (confirm('Tem certeza de que deseja excluir essa pessoa?')) {
            deleteUser(user.id);
          }
        });
      ...
      document.querySelector('.csv').addEventListener('click', function() {
          let data = [];
          const rows = document.querySelectorAll('.table-listarpessoa tr');
